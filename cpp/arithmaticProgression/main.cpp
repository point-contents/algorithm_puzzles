...
std::pair<long,int> findOddSpace(std::vector<long> elementDifferences)
{
	...
	{
		int occuranceCount = std::count(elementDifferences.begin(), elementDifferences.end(), steps);
	...
	auto itr = std::find(elementDifferences.begin(), elementDifferences.end(), found.m_uniqueSpace);
	...
	std::cout << "Missing Element Index - 1 is: " << found.m_uniqueSpaceIndex << std::endl;
...
int findMissing(std::vector<long> list)
{
	...
	std::cout << "Pair Contents " << missingSpotAndIterator.first << ":" << missingSpotAndIterator.second << std::endl;
